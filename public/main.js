...
function drawClock() {
	...
	var hour = d.getHours();
	{
		...
		ctx2.font = size + 'px' + "'Paytone One'";
		ctx2.font = size + 'px' + "'Offside'";
		ctx2.font = size + 'px' + "'Ropa Sans'";
		ctx2.font = size + 'px' + "'Open Sans Condensed'";
		ctx2.font = size + 'px' + "'Dosis'";
		*/

		/*
		var size = 250;
		var interval = size - 100;
		var xpos = 0;
		...
		mojiCode = Math.floor((sec / 10) % 10 + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText, xpos + interval * 2, ypos);

		mojiCode = Math.floor((sec % 10) + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText + "'", xpos + interval * 3, ypos);

		mojiCode = Math.floor((ms / 100) % 10 + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText, xpos + interval * 4, ypos);
		*/
		...
		mojiCode = Math.floor((hour / 10) % 10 + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText, xpos + interval * 0, ypos);

		mojiCode = Math.floor((hour % 10) + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText + ":", xpos + interval * 1, ypos);
		
		mojiCode = Math.floor((min / 10) % 10 + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText, xpos + interval * 2, ypos);

		mojiCode = Math.floor((min % 10) + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText + ":", xpos + interval * 3, ypos);
		...
		mojiCode = Math.floor((sec % 10) + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText + "'", xpos + interval * 5, ypos);

		mojiCode = Math.floor((ms / 100) % 10 + 48);
		aText = String.fromCharCode(mojiCode);
		ctx2.fillText(aText, xpos + interval * 6, ypos);
	}
}
...
function drawLeap() {
	...
	for (var i = 0; i < leapFrame.hands.length; i++) {
		for (var j = 0; j < leapFrame.hands[i].fingers.length; j++) {
			...
			ctx.beginPath();
			ctx.arc(x, y, z*0.7, 0, Math.PI*2, false);
			ctx.fill();
		}
	}
}
...
*/
	...
	if (fftOn) {
		...
		for (var i = 0; i < fftBars; i++) {
			...
			ctx2.fillRect(xlog, 600, 2, -magnitude);
		}
	}
	drawMask();
	drawLeap();
